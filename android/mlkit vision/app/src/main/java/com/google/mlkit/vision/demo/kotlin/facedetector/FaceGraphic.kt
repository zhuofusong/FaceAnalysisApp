 ...
 */
class FaceGraphic constructor(overlay: GraphicOverlay?, private val face: Face, private val findex: DoubleArray) : Graphic(overlay) {
  ...
  override fun draw(canvas: Canvas) {
    ...
    if (face.getContour(FaceContour.LEFT_EYE) != null) {
      ...
      textWidth = max(
        ...
        idPaints[colorID].measureText(
          String.format(
            Locale.US, "Canthal index: %.2f", findex[0]
          )
        )
      )
    }
    ...
    // Draws all kinds of face indices
    if (face.getContour(FaceContour.LEFT_EYE) != null){
      canvas.drawText("Canthal index: " + String.format(Locale.US, "%.2f", findex[0]),
        left,
        top + yLabelOffset,
        idPaints[colorID]
      )
